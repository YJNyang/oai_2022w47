...
{
      FILE *fpRead = fopen("./dpssSeq.txt","r");
      if(NULL ==  fpRead){
...
void init_context_pss_nr(NR_DL_FRAME_PARMS *frame_parms_ue)
{
  ...
  for (int i = 0; i < NUMBER_DPSS_SEQUENCE*NUMBER_PSS_SEQUENCE; i++){
   ...
   /* precfo_seq memory alloc */
  primary_synchro_time_nr_fo = malloc16(size);
  if (primary_synchro_time_nr_fo == NULL) {
    LOG_E(PHY,"Fatal memory allocation problem \n");
    assert(0);
  }
  bzero( primary_synchro_time_nr_fo, size);
...
void free_context_pss_nr(void)
{
  ...
  free_and_zero(primary_synchro_time_nr_fo);
}
...
// ===========================================  SNR计算   ===========================================
  ...
  if (syncSNR < 8) {/* SNR */
    LOG_I(PHY,"[TRACK SYNC] Sync SNR = %d is too low, can't find PSS Position.\n",syncSNR);
    return(-1);
  }
    ...
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> PSS IFO EST <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<//
      ...
      for (int fo_idx = -4; fo_idx < 5 ; fo_idx++){
       ...
       double fo_angle = -2*M_PI*(fo_idx+ffo_cp)/frame_parms->ofdm_symbol_size;  //搜索区间：[-4,4]整数
        pre_cfo_compensation_simd((int32_t *)primary_synchro_time_nr[pss_source],(int32_t *)primary_synchro_time_nr_fo,0,frame_parms->ofdm_symbol_size,fo_angle);
        // int ifo_est=fo_idx-4;
        /* 频偏估计等效序列相关运算 */
        result  = dot_product64((short*)primary_synchro_time_nr_fo, 
...
int pss_freq_track_nr(int **rxdata, ///rx data in time domain
                       NR_DL_FRAME_PARMS *frame_parms,
                       int position,
                       int eNB_id,
		                   int *f_off)
